...
//#region src/app/components/nuxt-announcer.ts
const NuxtAnnouncer = defineComponent({
	...
	setup(props, { slots, expose }) {
		const { set, polite, assertive, message, politeness } = useAnnouncer({ politeness: props.politeness });
		const role = computed(() => {
			if (politeness.value === "assertive") return "alert";
			if (politeness.value === "off") return;
			return "status";
		});
		expose({
			set,
