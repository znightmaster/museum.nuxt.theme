...
//#region src/app/components/client-fallback.server.ts
const NuxtClientFallbackServer = defineComponent({
	...
	async setup(_, ctx) {
		...
		try {
			...
			if (defaultSlot) for (let i = 0; i < defaultSlot.length; i++) ssrRenderVNode(ssrVNodes.push, defaultSlot[i], vm);
			const buffer = ssrVNodes.getBuffer();
			if (buffer.hasAsync) await Promise.all(buffer.flat(Infinity).filter(isPromise));
			return {
				ssrFailed,
				ssrVNodes
			};
		} catch (ssrError) {
			if (import.meta.dev) renderDiagnostics.NUXT_E4006({ cause: ssrError });
			error.value = true;
	...
	ssrRender(ctx, push, parent) {
		if (ctx.ssrFailed) {
			...
			if (fallback || placeholder) ssrRenderSlot(ctx.$slots, fallback ? "fallback" : "placeholder", {}, null, push, parent);
			else {
