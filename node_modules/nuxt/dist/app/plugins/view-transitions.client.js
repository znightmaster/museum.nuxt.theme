...
import { appViewTransition } from "#build/nuxt.config.mjs";
//#region src/app/plugins/view-transitions.client.ts
const plugin = /* @__PURE__ */ defineNuxtPlugin((nuxtApp) => {
	...
	const normalizeViewTransitionOptions = (value) => {
		if (typeof value === "boolean" || value === "always") return { enabled: value };
	...
	router.beforeResolve(async (to, from) => {
		...
		const viewTransitionMode = toViewTransitionOptions.enabled ?? appViewTransition.enabled;
		const prefersNoTransition = window.matchMedia("(prefers-reduced-motion: reduce)").matches && viewTransitionMode !== "always";
		...
		const viewTransitionBaseTypes = resolveViewTransitionTypes(toViewTransitionOptions.types) ?? resolveViewTransitionTypes(appViewTransition.types) ?? [];
		...
		transition = allTypes.length > 0 ? document.startViewTransition({
			update,
			types: allTypes
