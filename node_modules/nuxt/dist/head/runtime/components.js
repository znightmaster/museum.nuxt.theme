...
//#region src/head/runtime/components.ts
...
function normalizeProps(_props, key) {
	const props = Object.fromEntries(Object.entries(_props).filter(([_, value]) => value !== void 0));
	if (typeof props.body !== "undefined") props.tagPosition = props.body ? "bodyClose" : "head";
	if (typeof props.renderPriority !== "undefined") props.tagPriority = props.renderPriority;
	return {
		...props,
		key
	};
...
const NoScript = defineComponent({
	...
	setup(props, { slots }) {
		...
		return () => {
			...
			const textContent = [];
			if (slotVnodes) {
				for (const vnode of slotVnodes) if (vnode.children) textContent.push(vnode.children);
			}
			if (textContent.length > 0) noscript.textContent = textContent.join("");
...
const Link = defineComponent({
	...
	setup(props) {
		const { input, update } = useHeadComponentCtx();
		input.link ||= [];
		const idx = input.link.push(null) - 1;
		const key = useVNodeStringKey();
		onUnmounted(() => {
			input.link[idx] = null;
			update();
...
const Title = defineComponent({
	...
	setup(_, { slots }) {
		...
		return () => {
			...
			if (import.meta.dev) {
				if (defaultSlot && (defaultSlot.length > 1 || defaultSlot[0] && typeof defaultSlot[0].children !== "string")) unheadDiagnostics.NUXT_E6002();
			}
			update();
			return null;
		};
...
const Meta = defineComponent({
	...
	setup(props) {
		...
		const key = useVNodeStringKey();
		input.meta ||= [];
		const idx = input.meta.push(null) - 1;
		onUnmounted(() => {
			input.meta[idx] = null;
			update();
		});
		return () => {
			const meta = {
				...
				...normalizeProps(props, key)
			};
			if ("httpEquiv" in meta) delete meta.httpEquiv;
			input.meta[idx] = meta;
			update();
			return null;
		};
...
const Style = defineComponent({
	...
	setup(props, { slots }) {
		...
		return () => {
			...
			if (textContent) {
				if (import.meta.dev && typeof textContent !== "string") unheadDiagnostics.NUXT_E6003();
