...
import fs, { existsSync, mkdirSync, promises, readdirSync, statSync, writeFileSync } from "node:fs";
...
import { genArrayFromRaw, genDynamicImport, genDynamicTypeImport, genExport, genImport, genInlineTypeImport, genObjectFromRawEntries, genObjectKey, genSafeVariableName, genString } from "knitwork";
...
import { ELEMENT_NODE, parse as parse$1, walk as walk$1 } from "ultrahtml";
...
function getNameFromPath(path, relativeTo) {
	...
	return kebabCase(resolveComponentNameSegments(fileName.toLowerCase() === "index" ? "" : fileName, prefixParts).filter(Boolean)).replace(QUOTE_RE, "");
}
...
const PAGE_EXTRACT_RE = new RegExp(`\\b(${["definePageMeta", "defineRouteRules"].join("|")})\\b`, "g");
...
function getRouteMeta(contents, absolutePath, extraExtractionKeys = /* @__PURE__ */ new Set()) {
	...
	for (const script of scriptBlocks) {
		const found = {};
		for (const macro of script.code.matchAll(PAGE_EXTRACT_RE)) found[macro[1]] = false;
		...
		parseAndWalk(script.code, absolutePath.replace(/\.\w+$/, "." + script.loader), (node) => {
			...
			if (pageExtractArgument?.type !== "ObjectExpression") {
				pageDiagnostics.NUXT_B4005({
					fnName,
					file: absolutePath,
					receivedType: String(pageExtractArgument?.type)
				});
				return;
			}
			...
			if (fnName === "definePageMeta") {
				for (const key of extractionKeys) {
					const property = pageExtractArgument.properties.find((property) => property.type === "Property" && property.key.type === "Identifier" && property.key.name === key);
					...
					} else extractedData[key] = value;
				}
				for (const property of pageExtractArgument.properties) {
					if (property.type !== "Property") continue;
					if (!(property.key.type === "Literal" || property.key.type === "Identifier")) continue;
...
function normalizeRoutes(routes, metaImports = /* @__PURE__ */ new Set(), options) {
	...
	return {
		...
		routes: genArrayFromRaw(routes.map((page) => {
			...
			metaImports.add(genImport(`${file}?macro=true`, [{
				name: "default",
				as: metaImportName
			}]));
	...
	for (const page of pages) {
		if (page.rules) {
			if (Object.keys(page.rules).length) {
				...
				if (glob) paths[glob] = page.rules;
			}
			delete page.rules;
		}
		if (page.children?.length) globRouteRulesFromPages(page.children, paths, prefix + page.path + "/");
	}
	return paths;
}
				...
				function processDeclaration(scopeTrackerNode) {
					if (scopeTrackerNode?.type === "Variable") {
						...
						for (const decl of scopeTrackerNode.variableNode.declarations) {
							...
							walk(decl.init, { enter: (node, parent) => {
						...
						if (meta.type === "ObjectExpression") {
							...
							for (let i = 0; i < meta.properties.length; i++) {
								...
								if (options.extractedKeys?.includes(prop.key.name)) {
									...
									if (serializable) omitProp(prop, i);
...
var module_default$4 = defineNuxtModule({
	...
	async setup(_options, nuxt) {
		...
		const resolvePagesRoutes$1 = async (pattern, nuxt) => {
			const pages = await resolvePagesRoutes(pattern, nuxt, pagesCtx);
			...
			const typedRouterOptions = {
				...
				async beforeWriteFiles(rootPage) {
					...
					function addPage(parent, page, basePath = "") {
						...
						if (page.name) route.name = page.name;
						if (page.children) for (const child of page.children) addPage(route, child, absolutePagePath);
					}
		...
		nuxt.hook("nitro:build:before", (nitro) => {
			...
			if (!nitro.options.static) {
				...
				for (const [route, rules] of Object.entries(nitro.options.routeRules)) addRoute(routeRulesRouter, void 0, route, rules);
				for (const route of prerenderRoutes) if (defu({}, ...findAllRoutes(routeRulesRouter, void 0, route).reverse()).prerender) nitro.options.prerender.routes.push(route);
			}
		...
		const extraPageMetaExtractionKeys = nuxt.options?.experimental?.extraPageMetaExtractionKeys || [];
		const extractedKeys = [...defaultExtractionKeys, ...extraPageMetaExtractionKeys];
...
var module_default$3 = defineNuxtModule({
	...
	setup(options, nuxt) {
		...
		const unheadLegacy = resolveModulePath("@unhead/vue/legacy", {
			try: true,
			from: importPaths
		}) || "@unhead/vue/legacy";
		const unheadPlugins = resolveModulePath("@unhead/vue/plugins", {
			try: true,
			from: importPaths
		}) || "@unhead/vue/plugins";
		addTemplate({
...
const IslandsTransformPlugin = (options) => createUnplugin((_options, meta) => {
	...
	return {
		...
		transform: {
			filter: { code: { include: [HAS_SLOT_OR_CLIENT_RE] } },
			async handler(code, id, transformMeta) {
				...
				if (!SCRIPT_RE$3.test(code)) s.prepend("<script setup>\nimport { mergeProps as __mergeProps } from 'vue'\nimport { vforToArray as __vforToArray } from '#app/components/utils'\nimport NuxtTeleportIslandComponent from '#app/components/nuxt-teleport-island-component'\nimport NuxtTeleportSsrSlot from '#app/components/nuxt-teleport-island-slot'<\/script>");
				else for (const match of code.matchAll(SCRIPT_RE_GLOBAL)) s.appendRight(match.index + match[0].length, IMPORT_CODE);
				let hasNuxtClient = false;
				await walk$1(parse$1(template[0]), (node) => {
					...
					if (node.name === "slot") {
						...
						} else s.overwrite(startingIndex + loc[0].start, startingIndex + loc[0].end, code.slice(startingIndex + loc[0].start, startingIndex + loc[0].end).replaceAll(EXTRACTED_ATTRS_RE, ""));
						s.appendRight(startingIndex + loc[1].end, "</NuxtTeleportSsrSlot>");
						return;
					}
					if (node.name === "NuxtTeleportIslandComponent") return;
					if (!("nuxt-client" in node.attributes) && !(":nuxt-client" in node.attributes)) return;
					hasNuxtClient = true;
				...
				const { program: ast } = parseAndWalk(code, id, (node) => {
					...
					if (componentCall.type === "Identifier" || componentCall.type === "MemberExpression" || componentCall.type === "CallExpression") {
						...
						for (const slot of slotsToRemove) {
							...
							parseAndWalk(`({${code.slice(slot.start, slot.end + 1)}})`, id, (node) => {
								if (!isSsrRender(node)) return;
				...
				try {
					...
					await walk$1(ast, (node) => {
						...
						for (const attr in node.attributes) {
							const prop = camelCase(attr.startsWith(":") ? attr.slice(1) : attr);
	...
	async setup(moduleOptions, nuxt) {
		...
		nuxt.hook("app:resolve", async () => {
			...
			for (const dir of allDirs) {
				...
				(dirPath.includes("node_modules") ? libraryComponentDirs : userComponentDirs).push({
						...
						processKeyedFunction(this, node, ({ parsedCall, fnMeta }) => {
							...
							switch (parsedCall.name) {
								...
								case "useLazyAsyncData":
									if (stringTypes.includes(parsedCall.callExpression.arguments[0]?.type) && typeof fnMeta.source === "string" && stripExtension(fnMeta.source) === stripExtension(resolveAlias("#app/composables/asyncData", options.alias))) return;
									break;
							}
...
async function getVueHash(nuxt) {
	...
	return {
		...
		async restoreCache() {
			...
			const elapsed = Date.now() - start;
			if (res) consola.success(`Restored Vue client and server builds from cache in \`${elapsed}ms\`.`);
			return res;
		}
	};
}
...
async function restoreCachedBuildId(nuxt) {
	...
	const buildIdCacheFile = join(getCacheDir(nuxt), "vue", hash + ".buildid");
...
async function writeCache(cwd, sources, cacheFile) {
	const tarData = createTar(await readFilesRecursive(sources, {
		patterns: ["**/*", "!analyze/**"],
		cwd
	}));
...
var schema_default = defineNuxtModule({
	...
	async setup(_, nuxt) {
		...
		async function resolveSchema$1() {
			...
			const schema = defu(...await Promise.all(schemaDefs.map((schemaDef) => resolveSchema(schemaDef))));
...
const RemovePluginMetadataPlugin = (nuxt) => createUnplugin(() => {
	return {
		...
		transform(code, id, meta) {
			...
			if (!findExports(code).find((e) => e.type === "default" || e.name === "default")) {
...
async function initNuxt(nuxt) {
	...
	if (nuxt.options.experimental.emitRouteChunkError === "automatic-immediate") addPlugin(resolve(nuxt.options.appDir, "plugins/chunk-reload-immediate.client"));
	if (nuxt.options.experimental.emitRouteChunkError) addPlugin(resolve(nuxt.options.appDir, "plugins/chunk-reload-crawler.client"));
...
async function resolveModules(nuxt) {
	...
	for (const config of configs) {
		...
		const layerModules = await resolveFiles(resolve(config.srcDir, (config.rootDir === nuxt.options.rootDir ? nuxt.options.dir : config.dir)?.modules || "modules"), [`*{${nuxt.options.extensions.join(",")}}`, `*/index{${nuxt.options.extensions.join(",")}}`]);
...
const islandRendererTemplate = {
	...
	getContents(ctx) {
		...
		return [
			"import { defineAsyncComponent } from 'vue'",
			`const IslandRenderer = import.meta.server ? defineAsyncComponent(() => ${genDynamicImport(resolve(ctx.nuxt.options.appDir, "components/island-renderer"), { wrapper: false })}.then(r => r.default || r)) : () => null`,
			"export default IslandRenderer"
		].join("\n");
	}
};
...
function checkForCircularDependencies(_plugins) {
	...
	for (const plugin of _plugins) {
		if (plugin.dependsOn && plugin.dependsOn.some((name) => !pluginNames.has(name))) pluginDiagnostics.NUXT_B2008({
			name: plugin.name,
			missing: plugin.dependsOn.filter((name) => !pluginNames.has(name)).join(", ")
		});
		if (plugin.name) deps[plugin.name] = plugin.dependsOn || [];
	}
